...
感谢你对 ResearchClaw 的关注！ResearchClaw 是一个开源的 **AI 科研助手**，可以在你自己的环境中运行——无论是你的机器还是云端。它帮助研究人员追踪论文、管理文献、记录实验、分析数据。它可以连接钉钉、飞书、QQ、Discord、iMessage 等聊天应用，支持定时任务和心跳机制，并通过 **Skills** 扩展其能力。我们热烈欢迎能让 ResearchClaw 对所有人更有用的贡献：无论是添加新的频道、新的模型提供商、Skill，改进文档，还是修复 bug。

**快速链接：** [GitHub](https://github.com/MingxinYang/ResearchClaw) · [许可证：Apache 2.0](LICENSE)
...
- **检查 [Open Issues](https://github.com/MingxinYang/ResearchClaw/issues)** 以及任何 [Projects](https://github.com/MingxinYang/ResearchClaw/projects) 或路线图标签。
...
ResearchClaw 设计为**可扩展的**：你可以添加模型、频道、Skills 等。以下是我们关心的主要贡献领域。

---

### 添加新模型 / 模型提供商

ResearchClaw 支持**多种模型后端**：云 API（如 DashScope、ModelScope）、**Ollama** 和本地后端（**llama.cpp**、**MLX**）。你可以通过两种方式贡献：
...
1. **提供商定义**（在 `src/researchclaw/providers/registry.py` 或等效位置）：
...
2. **聊天模型类**（如果 API 不兼容 OpenAI）：
   ...
   - 在注册表的聊天模型映射中注册该类，以便运行时可以按名称解析它（参见 `src/researchclaw/providers/registry.py` 中的 `_CHAT_MODEL_MAP`）。
...
频道是 ResearchClaw 与**钉钉、飞书、QQ、Discord、iMessage** 等通信的方式。你可以添加新频道，以便 ResearchClaw 可以与你喜欢的 IM 或机器人平台配合使用。
...
- **实现：** 实现 **`BaseChannel` 的子类**（在 `src/researchclaw/app/channels/base.py` 中）：
...
- **发现：** 内置频道在 `src/researchclaw/app/channels/registry.py` 中注册。**自定义频道**从工作目录加载：放置一个模块（如 `custom_channels/telegram.py` 或包 `custom_channels/telegram/`），定义一个带有 `channel` 属性的 `BaseChannel` 子类。
- **CLI：** 用户使用以下命令安装/添加频道：
  - `researchclaw channels install <key>` — 创建模板或从 `--path` / `--url` 复制
  - `researchclaw channels add <key>` — 安装并添加到配置
  - `researchclaw channels remove <key>` — 从 `custom_channels/` 中删除自定义频道
  - `researchclaw channels config` — 交互式配置
...
**Skills** 定义了 ResearchClaw 可以做什么：cron、文件读取、PDF/Office、论文搜索、文献管理等。我们欢迎**广泛有用的**基础 skills（科研、生产力、文档、通信、自动化），适合大多数用户。
...
- **位置：** 内置 skills 位于 `src/researchclaw/agents/skills/<skill_name>/` 下。应用程序将内置和用户的 **customized_skills**（来自工作目录）合并到 **active_skills** 中；除了在目录中放置有效的 `SKILL.md` 外，不需要额外的注册。
...
3. **具体描述功能范围**：说明技能做什么，不要含糊
   - ✅ 好的："Search and track academic papers from ArXiv and Semantic Scholar"
   - ❌ 不好："Search things"
...
| 技能 | 描述（不好） | 描述（好） |
|------|---------------|-------------|
| Paper Search | "搜索论文" | "Use this skill whenever user wants to search academic papers or track new publications. Trigger especially when user mentions: \"paper\", \"arxiv\", \"publication\", \"search papers\", or requests for literature review." |
...
ResearchClaw 旨在在 **Windows**、**Linux** 和 **macOS** 上运行。欢迎改进特定平台支持的贡献。

- **兼容性修复：** 路径处理、行尾、shell 命令或在不同操作系统上行为不同的依赖项。
- **安装和运行：** 一行安装（`install.sh`）、`pip` 安装，以及 `researchclaw init` / `researchclaw app` 应该在每个支持的平台上工作（或有清晰的文档说明）。
- **平台特定功能：** 可选集成是可以的，只要它们不会破坏其他平台。在适当的地方使用运行时检查或可选依赖项。
- **文档：** 在文档或 README 中记录任何平台特定的步骤、已知限制或推荐设置。

如果你添加或更改平台支持，请在受影响的操作系统上进行测试，并在 PR 描述中提及。

---

### 其他贡献

- **MCP（模型上下文协议）：** ResearchClaw 支持运行时 **MCP 工具**发现和热插拔。贡献新的 MCP 服务器或工具可以帮助用户扩展 agent 而无需更改核心代码。
- **文档：** 对文档（位于 `website/public/docs/` 下）和 README 的修复和改进始终受欢迎。
- **Bug 修复和重构：** 小的修复、更清晰的错误消息以及保持行为相同的重构都很有价值。对于较大的重构，最好先创建 issue。
- **示例和工作流程：** 教程或示例工作流程（如"每日论文摘要到钉钉"、"本地模型 + cron"）可以记录或从仓库/文档链接。
- **任何其他有用的东西！**
...
- **讨论：** [GitHub Discussions](https://github.com/MingxinYang/ResearchClaw/discussions)
- **Bug 和功能：** [GitHub Issues](https://github.com/MingxinYang/ResearchClaw/issues)

感谢你为 ResearchClaw 贡献代码。你的工作帮助它成为每个人更好的科研助手。🔬
