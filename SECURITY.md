...
If you believe you've found a security issue in MathClaw, please report it privately.

## Reporting

Report vulnerabilities of the MathClaw repository:

If you discover a security issue in MathClaw, please report it to us through [GitHub Security Advisories](https://github.com/MingxinYang/MathClaw/security/advisories).
...
7. **Environment** (Python version, OS, how MathClaw is run)
...
- Tested version details (MathClaw version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to MathClaw's documented trust boundaries (see below).
- For exposed-secret reports: proof the credential is MathClaw-owned or grants access to MathClaw-operated infrastructure/services.
...
- Reports that assume per-user multi-tenant authorization on a shared MathClaw instance/config.
...
## Alignment with `mathclaw init`

The security model and recommended baseline in this document are **aligned with** what users see and accept during **`mathclaw init`**. The init flow shows a security notice that covers: single-operator boundary; shared delegated authority when multiple people message the same instance; restricting channels and users (allowlists); using separate config/credentials and OS users or hosts per trust boundary; least privilege and sandboxing; keeping secrets out of the working directory and skill-accessible paths; and reviewing config and skills regularly. When updating either this document or the `SECURITY_WARNING` in `src/mathclaw/cli/init_cmd.py`, keep the **concepts and recommendations** consistent so operators get the same message in both places.

## Security & Trust

Security handling is owned by the MathClaw maintainers. For sensitive reports, use the private advisory or a private channel as above.

## Bug Bounties

MathClaw is a community open-source project. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly. The best way to help the project right now is by sending PRs or clear, reproducible reports.

## Operator Trust Model

MathClaw does **not** model one instance as a multi-tenant, adversarial user boundary.

- Authenticated callers to the same MathClaw instance (same config, same channel workspace) are treated as **trusted operators** for that instance.
...
- **Recommended mode**: one user per machine/host (or per OS user), one MathClaw config for that user, and one or more agents/skills inside that instance.
- If multiple users need MathClaw, use one host/OS user (or VPS) per user, or strict isolation; sharing one instance by mutually untrusted users is not the recommended default.
- Skills run with the same privileges as the MathClaw process; only install and enable skills you trust.


## Trusted Skills Concept

Skills/extensions are part of MathClaw's **trusted computing base** for an instance.
...
- Public internet exposure of MathClaw when the docs recommend against it.
- Using MathClaw in ways that the docs recommend not to.
- Deployments where mutually untrusted/adversarial operators share one MathClaw instance and config (e.g. reports expecting per-operator isolation for sessions, history, or similar).
...
- Exposed secrets that are third-party or user-controlled credentials (not MathClaw-owned and not granting access to MathClaw-operated infrastructure) without demonstrated MathClaw impact.
- Scanner-only claims against stale or nonexistent paths, or without a working repro.

## Deployment Assumptions

MathClaw security guidance assumes:

- The host where MathClaw runs is within a trusted OS/admin boundary.
- Anyone who can modify the MathClaw working directory and config (including `config.json`) is effectively a trusted operator.
...
MathClaw's security model is **"research assistant"** (one trusted operator, potentially many agents/skills), not "shared multi-tenant bus."

- If multiple people can message the same tool-enabled MathClaw instance (e.g. a shared channel workspace), they can all steer that agent within its granted permissions.
...
The MathClaw working directory is treated as **trusted local operator state**.
...
- Keep secrets out of the working directory and skill-accessible paths; this is part of the baseline recommended in `mathclaw init`.

## Skills Trust Boundary

Skills are loaded and run **in-process** (or under the same trust boundary) as the MathClaw runtime and are treated as trusted code.

- Skills can execute with the same OS privileges as the MathClaw process.
...
- **Python**: MathClaw requires a supported Python version (see [README](README.md)). Use a version with current security updates.
