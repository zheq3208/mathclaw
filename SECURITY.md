...
If you believe you've found a security issue in ResearchClaw, please report it privately.

## Reporting

Report vulnerabilities of the ResearchClaw repository:

If you discover a security issue in ResearchClaw, please report it to us through [GitHub Security Advisories](https://github.com/MingxinYang/ResearchClaw/security/advisories).
...
7. **Environment** (Python version, OS, how ResearchClaw is run)
...
- Tested version details (ResearchClaw version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to ResearchClaw's documented trust boundaries (see below).
- For exposed-secret reports: proof the credential is ResearchClaw-owned or grants access to ResearchClaw-operated infrastructure/services.
...
- Reports that assume per-user multi-tenant authorization on a shared ResearchClaw instance/config.
...
## Alignment with `researchclaw init`

The security model and recommended baseline in this document are **aligned with** what users see and accept during **`researchclaw init`**. The init flow shows a security notice that covers: single-operator boundary; shared delegated authority when multiple people message the same instance; restricting channels and users (allowlists); using separate config/credentials and OS users or hosts per trust boundary; least privilege and sandboxing; keeping secrets out of the working directory and skill-accessible paths; and reviewing config and skills regularly. When updating either this document or the `SECURITY_WARNING` in `src/researchclaw/cli/init_cmd.py`, keep the **concepts and recommendations** consistent so operators get the same message in both places.

## Security & Trust

Security handling is owned by the ResearchClaw maintainers. For sensitive reports, use the private advisory or a private channel as above.

## Bug Bounties

ResearchClaw is a community open-source project. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly. The best way to help the project right now is by sending PRs or clear, reproducible reports.

## Operator Trust Model

ResearchClaw does **not** model one instance as a multi-tenant, adversarial user boundary.

- Authenticated callers to the same ResearchClaw instance (same config, same channel workspace) are treated as **trusted operators** for that instance.
...
- **Recommended mode**: one user per machine/host (or per OS user), one ResearchClaw config for that user, and one or more agents/skills inside that instance.
- If multiple users need ResearchClaw, use one host/OS user (or VPS) per user, or strict isolation; sharing one instance by mutually untrusted users is not the recommended default.
- Skills run with the same privileges as the ResearchClaw process; only install and enable skills you trust.


## Trusted Skills Concept

Skills/extensions are part of ResearchClaw's **trusted computing base** for an instance.
...
- Public internet exposure of ResearchClaw when the docs recommend against it.
- Using ResearchClaw in ways that the docs recommend not to.
- Deployments where mutually untrusted/adversarial operators share one ResearchClaw instance and config (e.g. reports expecting per-operator isolation for sessions, history, or similar).
...
- Exposed secrets that are third-party or user-controlled credentials (not ResearchClaw-owned and not granting access to ResearchClaw-operated infrastructure) without demonstrated ResearchClaw impact.
- Scanner-only claims against stale or nonexistent paths, or without a working repro.

## Deployment Assumptions

ResearchClaw security guidance assumes:

- The host where ResearchClaw runs is within a trusted OS/admin boundary.
- Anyone who can modify the ResearchClaw working directory and config (including `config.json`) is effectively a trusted operator.
...
ResearchClaw's security model is **"research assistant"** (one trusted operator, potentially many agents/skills), not "shared multi-tenant bus."

- If multiple people can message the same tool-enabled ResearchClaw instance (e.g. a shared channel workspace), they can all steer that agent within its granted permissions.
...
- A team-shared agent can be valid when users are in the same trust boundary and the agent is strictly work-only.
- For team-shared setups, use a dedicated machine/VM/container and dedicated accounts; avoid mixing personal data on that runtime.
...
The ResearchClaw working directory is treated as **trusted local operator state**.
...
- Keep secrets out of the working directory and skill-accessible paths; this is part of the baseline recommended in `researchclaw init`.

## Skills Trust Boundary

Skills are loaded and run **in-process** (or under the same trust boundary) as the ResearchClaw runtime and are treated as trusted code.

- Skills can execute with the same OS privileges as the ResearchClaw process.
...
For more operational and hardening guidance, see the documentation and any security-related docs linked from the repo.

## Runtime Requirements

- **Python**: ResearchClaw requires a supported Python version (see [README](README.md)). Use a version with current security updates.
