...
def create_local_chat_model(
    ...
    ----------
    model_id:
        ID of a downloaded local model (from manifest).
    stream:
        Whether to use streaming output.
    backend_kwargs:
        Extra kwargs passed to the backend constructor
        (e.g., n_ctx, n_gpu_layers for llama.cpp).
    generate_kwargs:
        Extra kwargs passed to every generate call
    ...
    ------
    ValueError
        If model_id is not found in the manifest.
    ImportError
    ...
    if info is None:
        raise ValueError(
            ...
            "Download it first with 'researchclaw models download'.",
        )
