...
# 查看任务详情
mathclaw cron get <job_id>

# 查看任务状态
mathclaw cron state <job_id>

# 删除任务
mathclaw cron delete <job_id>

# 暂停/恢复任务
mathclaw cron pause <job_id>
mathclaw cron resume <job_id>

# 立即执行一次
mathclaw cron run <job_id>
```
...
```bash
mathclaw cron create -f job_spec.json
```
...
- 缺少参数时，询问用户补充后再创建
- 暂停/删除/恢复前，用 `mathclaw cron list` 查找 job_id
- 排查问题时，用 `mathclaw cron state <job_id>` 查看状态
- 给用户的命令要完整、可直接复制执行
