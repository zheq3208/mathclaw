...
def _resolve_base_url(base_url: str) -> str:
    ...
    env_url = os.environ.get("MATHCLAW_API_BASE_URL", "").strip()
    if env_url:
        return env_url.rstrip("/")

    host = os.environ.get("MATHCLAW_HOST", DEFAULT_HOST).strip() or DEFAULT_HOST
    if host in {"0.0.0.0", "::", "[::]"}:
        host = "127.0.0.1"
    port = str(os.environ.get("MATHCLAW_PORT", DEFAULT_PORT)).strip() or str(
        DEFAULT_PORT,
    )
