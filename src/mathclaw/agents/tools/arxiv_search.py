...
def arxiv_search(
    ...
    try:
        ...
        page_size = max(
            1,
            min(
                int(os.environ.get("MATHCLAW_ARXIV_PAGE_SIZE", "25") or "25"),
                max_results,
            ),
        )
        delay_seconds = max(
            1.0,
            float(
                os.environ.get("MATHCLAW_ARXIV_DELAY_SECONDS", "3.0") or "3.0",
            ),
        )
        retries = max(
            0,
            int(os.environ.get("MATHCLAW_ARXIV_NUM_RETRIES", "2") or "2"),
        )
...
def arxiv_download(
    ...
    output_dir:
        Directory to save the PDF. Defaults to ``~/.mathclaw/papers/``.
