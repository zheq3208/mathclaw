...
Shared by in-chat slash commands and CLI `mathclaw daemon <sub>`.
"""
...
async def run_daemon_restart(context: DaemonContext) -> str:
    ...
    return (
        ...
        "Use `mathclaw app` runtime control or restart the process manually."
    )
...
def run_daemon_version(context: DaemonContext) -> str:
    return (
        ...
        f"- Log file: {context.working_dir / 'mathclaw.log'}"
    )


def run_daemon_logs(context: DaemonContext, lines: int = 100) -> str:
    content = _get_last_lines(context.working_dir / "mathclaw.log", lines=lines)
