...
def _create_from_drafts(drafts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    from mathclaw.agents.skills_manager import SkillsManager
    from mathclaw.app.services.skill_creator import materialize_preview_drafts
...
async def preview_skills(request: SkillPreviewRequest):
    ...
    try:
        from mathclaw.app.services.skill_creator import preview_markdown_skills
...
async def install_skill(request: SkillInstallRequest):
    ...
    try:
        from mathclaw.agents.skills_hub import SkillsHubClient
