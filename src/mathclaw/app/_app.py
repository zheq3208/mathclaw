"""MathClaw FastAPI application entry point.
...
from ..utils.logging import add_mathclaw_file_handler
...
async def lifespan(app: FastAPI):
    ...
    logger.info("MathClaw v%s starting up...", __version__)
    add_mathclaw_file_handler(Path(WORKING_DIR) / "mathclaw.log")
    ...
    # Shutdown
    logger.info("MathClaw shutting down...")
...
async def get_version():
    ...
    return {"version": __version__, "name": "MathClaw"}
...
_router_defs: list[tuple[str, str, list[str]]] = [
    ("mathclaw.app.routers.agent", "/api/agent", ["Agent"]),
    ("mathclaw.app.routers.config", "/api/config", ["Config"]),
    ("mathclaw.app.routers.console", "/api/console", ["Console"]),
    ("mathclaw.app.routers.control", "/api/control", ["Control"]),
    ("mathclaw.app.routers.envs", "/api/envs", ["Environments"]),
    (
        "mathclaw.app.routers.local_models",
        "/api",
        ["LocalModels"],
    ),
    ("mathclaw.app.routers.mcp", "/api/mcp", ["MCP"]),
    ("mathclaw.app.routers.memory", "/api/memory", ["Memory"]),
    (
        "mathclaw.app.routers.ollama_models",
        "/api",
        ["OllamaModels"],
    ),
    ("mathclaw.app.routers.papers", "/api/papers", ["Papers"]),
    ("mathclaw.app.routers.providers", "/api/providers", ["Providers"]),
    ("mathclaw.app.routers.skills", "/api/skills", ["Skills"]),
    ("mathclaw.app.routers.workspace", "/api/workspace", ["Workspace"]),
]
...
for _mod_path, _prefix, _tags in [
    ("mathclaw.app.crons.api", "/api/crons", ["Crons"]),
    ("mathclaw.app.runner.api", "/api/runner", ["Runner"]),
]:
    try:
        import importlib as _il
