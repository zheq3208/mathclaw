...
def configure_providers_interactive(*, use_defaults: bool = False) -> None:
    ...
    if use_defaults:
        click.echo(
            "Using default provider settings. Run 'mathclaw models config' to change.",
        )
        return
...
def list_cmd() -> None:
    ...
    if not providers:
        ...
        click.echo("Run 'mathclaw models config' to add one.")
        return
...
def download_cmd(
    ...
    \b
    Examples:
      mathclaw models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
      mathclaw models download Qwen/Qwen3-8B-GGUF -f qwen3-8b.Q4_K_M.gguf
    """
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'mathclaw[local]'",
                fg="red",
            ),
        )
...
def list_local_cmd(backend: str | None) -> None:
    ...
    try:
        ...
        click.echo(
            "Local model support not installed. Install with: pip install 'mathclaw[local]'",
        )
        return
    ...
    if not models:
        ...
        click.echo(
            "Use 'mathclaw models download <repo_id>' to download one.",
        )
        return
