...
from mathclaw.app.channels.base import BaseChannel
from mathclaw.app.channels.schema import ChannelType
...
def list_cmd() -> None:
    ...
    if not channels:
        click.echo(
            "No channels configured. Run 'mathclaw channels config' to add one.",
        )
        return
...
def install_cmd(key: str, from_path: str | None, from_url: str | None) -> None:
    ...
    click.echo(
        f"✓ Created {dest_file}. Edit and add config with 'mathclaw channels config'.",
    )
