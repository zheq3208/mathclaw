"""mathclaw uninstall — remove the environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """Remove MathClaw PATH lines from a shell profile."""
    if not profile.is_file():
        return False
    text = profile.read_text()
    cleaned = re.sub(
        r"\n?# MathClaw\nexport PATH=\"\$HOME/\.mathclaw/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove MathClaw environment, CLI wrapper, and shell PATH entries."""
    wd = Path(WORKING_DIR)

    if purge:
        click.echo(f"This will remove ALL MathClaw data in {wd}")
    else:
        click.echo(
            "This will remove the MathClaw Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("MathClaw uninstalled. Please restart your terminal.")
