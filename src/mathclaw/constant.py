...
LOG_LEVEL_ENV = "MATHCLAW_LOG_LEVEL"

# ── Working directory ───────────────────────────────────────────────────────

WORKING_DIR: str = os.environ.get(
    "MATHCLAW_WORKING_DIR",
    str(Path.home() / ".mathclaw"),
)
...
JOBS_FILE: str = os.environ.get("MATHCLAW_JOBS_FILE", "jobs.json")
CHATS_FILE: str = os.environ.get("MATHCLAW_CHATS_FILE", "chats.json")
CONFIG_FILE: str = os.environ.get("MATHCLAW_CONFIG_FILE", "config.json")
ENV_FILE: str = os.environ.get("MATHCLAW_ENV_FILE", ".env")

# ── Skills directories ──────────────────────────────────────────────────────

ACTIVE_SKILLS_DIR: str = os.environ.get(
    "MATHCLAW_ACTIVE_SKILLS_DIR",
    str(Path(WORKING_DIR) / "active_skills"),
)
CUSTOMIZED_SKILLS_DIR: str = os.environ.get(
    "MATHCLAW_CUSTOMIZED_SKILLS_DIR",
...
MEMORY_COMPACT_KEEP_RECENT: int = int(
    os.environ.get("MATHCLAW_MEMORY_COMPACT_KEEP_RECENT", "3"),
)
MEMORY_COMPACT_RATIO: float = float(
    os.environ.get("MATHCLAW_MEMORY_COMPACT_RATIO", "0.7"),
)

# ── Heartbeat / Cron defaults ──────────────────────────────────────────────

HEARTBEAT_INTERVAL_MINUTES: int = int(
    os.environ.get("MATHCLAW_HEARTBEAT_INTERVAL", "60"),
)
HEARTBEAT_ENABLED: bool = (
    os.environ.get("MATHCLAW_HEARTBEAT_ENABLED", "true").lower() == "true"
)

# ── Paper digest defaults ──────────────────────────────────────────────────

PAPER_DIGEST_HOUR: int = int(
    os.environ.get("MATHCLAW_PAPER_DIGEST_HOUR", "8"),
)
PAPER_DIGEST_ENABLED: bool = (
    os.environ.get("MATHCLAW_PAPER_DIGEST_ENABLED", "false").lower()
    == "true"
)

# ── Server configuration ──────────────────────────────────────────────────

DEFAULT_HOST: str = os.environ.get("MATHCLAW_HOST", "127.0.0.1")
DEFAULT_PORT: int = int(os.environ.get("MATHCLAW_PORT", "8088"))

DOCS_ENABLED: bool = (
    os.environ.get("MATHCLAW_DOCS_ENABLED", "false").lower() == "true"
)
CORS_ORIGINS: str = os.environ.get("MATHCLAW_CORS_ORIGINS", "")
...
SKILLS_HUB_URL: str = os.environ.get(
    "MATHCLAW_SKILLS_HUB_URL",
    "https://hub.researchclaw.io",
)
SKILLS_HUB_TIMEOUT: int = int(
    os.environ.get("MATHCLAW_SKILLS_HUB_TIMEOUT", "15"),
)
SKILLS_HUB_RETRIES: int = int(
    os.environ.get("MATHCLAW_SKILLS_HUB_RETRIES", "3"),
)
