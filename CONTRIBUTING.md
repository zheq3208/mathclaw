# Contributing to MathClaw

## Welcome! 🔬

Thank you for your interest in contributing to MathClaw! MathClaw is an open-source **AI research assistant** that runs in your own environment—on your machine or in the cloud. It helps researchers track papers, manage references, log experiments, and analyze data. It connects to DingTalk, Feishu, QQ, Discord, iMessage, and other chat apps, supports scheduled tasks and heartbeat, and extends its capabilities through **Skills**. We warmly welcome contributions that help make MathClaw more useful for everyone: whether you add a new channel, a new model provider, a Skill, improve docs, or fix bugs.

**Quick links:** [GitHub](https://github.com/MingxinYang/MathClaw) · [License: Apache 2.0](LICENSE)
...
- **Check [Open Issues](https://github.com/MingxinYang/MathClaw/issues)** and any [Projects](https://github.com/MingxinYang/MathClaw/projects) or roadmap labels.
...
MathClaw is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

MathClaw supports **multiple model backends**: cloud APIs (e.g. DashScope, ModelScope), **Ollama**, and local backends (**llama.cpp**, **MLX**). You can contribute in two ways:
...
1. **Provider definition** (in `src/mathclaw/providers/registry.py` or equivalent):
...
2. **Chat model class** (if the API is not OpenAI-compatible):
   ...
   - Register the class in the registry's chat model map so the runtime can resolve it by name (see `_CHAT_MODEL_MAP` in `src/mathclaw/providers/registry.py`).
...
Channels are how MathClaw talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so MathClaw can work with your favorite IM or bot platform.
...
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/mathclaw/app/channels/base.py`):
...
- **Discovery:** Built-in channels are registered in `src/mathclaw/app/channels/registry.py`. **Custom channels** are loaded from the working directory: place a module (e.g. `custom_channels/telegram.py` or a package `custom_channels/telegram/`) that defines a `BaseChannel` subclass with a `channel` attribute.
- **CLI:** Users install/add channels with:
  - `mathclaw channels install <key>` — create a template or copy from `--path` / `--url`
  - `mathclaw channels add <key>` — install and add to config
  - `mathclaw channels remove <key>` — remove custom channel from `custom_channels/`
  - `mathclaw channels config` — interactive config
...
**Skills** define what MathClaw can do: cron, file reading, PDF/Office, paper search, reference management, etc. We welcome **broadly useful** base skills (research, productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/mathclaw/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
MathClaw aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.

- **Compatibility fixes:** Path handling, line endings, shell commands, or dependencies that behave differently per OS.
- **Install and run:** One-line install (`install.sh`), `pip` install, and `mathclaw init` / `mathclaw app` should work (or be clearly documented) on each supported platform.
...
- **MCP (Model Context Protocol):** MathClaw supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
...
- **Discussions:** [GitHub Discussions](https://github.com/MingxinYang/MathClaw/discussions)
- **Bugs and features:** [GitHub Issues](https://github.com/MingxinYang/MathClaw/issues)

Thank you for contributing to MathClaw. Your work helps make it a better research assistant for everyone. 🔬
